...
class Umi_Shortcodes {
	...
	public static function user_cabinet() {
		...
		// Avatar toggle row — kept outside the profile-info form to avoid nested <form> elements.
		$out .= '<div class="umi-form-row" style="text-align:center;margin-bottom:1rem">';
		$out .= '<button type="button" class="umi-cabinet-avatar-btn umi-cabinet-avatar-btn--clickable" data-umi-avatar-inline-toggle aria-label="' . esc_attr__( 'Изменить фото профиля', 'umi-marketplace' ) . '">';
		if ( $ava_url ) {
		...
		$out .= '</span></button>';
		$out .= '<div class="umi-avatar-inline" hidden style="display:none;text-align:left;margin-top:0.5rem">';
		...
		$out .= '<input type="hidden" class="umi-cabinet-attachment-id" name="umi_profile_photo_id" value="' . ( $photo_id > 0 ? (int) $photo_id : '' ) . '" />';
		...
		$out .= '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:0.5rem">';
		$out .= '<label class="umi-cabinet-file-label umi-btn umi-btn--secondary" data-umi-avatar-choose><input type="file" class="umi-cabinet-file" accept="image/jpeg,image/png,image/gif,image/webp" style="position:absolute;opacity:0;width:0;height:0" tabindex="-1" />';
		$out .= '<span>' . esc_html__( 'Выбрать изображение', 'umi-marketplace' ) . '</span></label>';
		$out .= '<div data-umi-avatar-save hidden style="display:none">';
		$out .= '<button type="submit" class="umi-btn umi-btn--primary">' . esc_html__( 'Сохранить', 'umi-marketplace' ) . '</button>';
		$out .= '</div>';
		$out .= '<button type="button" class="umi-btn umi-btn--outline" data-umi-avatar-inline-close>' . esc_html__( 'Отмена', 'umi-marketplace' ) . '</button>';
		$out .= '</div>';
		$out .= '</div></form>';
		$out .= '</div>';
		$out .= '</div>';
		// Profile info form — separate form, not wrapping the avatar row above.
